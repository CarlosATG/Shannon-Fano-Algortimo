...
def get_hex(Encoded):
    ...
    for i in range (len(Encoded)):#Queremos un rango igual al valor de la cantidad de valores que tenemos
        Hexalist.append(Encoded[i])#lo agregamos a la lista
    print(Hexalist)
    return Hexalist;
def FromTwo(Hexcoded):
    Hexalisto=[]#Creamos la lista
    for i in range (len(Hexcoded)):#Queremos un rango igual al valor de la cantidad de valores que tenemos
        Binary=int(Hexcoded[i], 2)#Encontramos su base 2
        Hexalisto.append(Binary)
    np_Hexalisto=np.array(Hexalisto)#Creamos un arreglo
    np_Hexalisto = np_Hexalisto.astype('H')#Lo hacemos base 16
    return np_Hexalisto;
...
Hexcoded= get_hex(Bincoded)#En binario
BitHexa= FromTwo(Hexcoded)#En Hexadecimal
Hexadict=dict(zip(Valores_en_Orden, Hexcoded))#Creamos un diccionario con el simbolo y su valor
print(Hexadict)
Decodeddict= dict(zip(Hexcoded, BitHexa))
print('Hola',Decodeddict)
...
for x in Codificado:
        Cadena+=''+x
print(Cadena)
bitCadena=bitarray(Cadena)
print(len(bitCadena),bitCadena)
